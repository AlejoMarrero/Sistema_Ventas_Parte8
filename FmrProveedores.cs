...
namespace Sistema_Ventas
{
    public partial class FmrProveedores : Form
    {
        ...
        private void FmrProveedores_Load(object sender, EventArgs e)
        {
            ...
            {
                dgwData.Rows.Add(new object[] {"",item.ID_Proveedor,item.Documento,item.Razon_Social,item.Correo,item.Telefono,
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                ID_Proveedor = string.IsNullOrEmpty(txtId.Text) ? 0 : Convert.ToInt32(txtId.Text),
            ...
            {
                int idgenerado = new Negocio_Proveedor().Registrar(obj, out Mensaje);

                if (idgenerado != 0)
                {
                    dgwData.Rows.Add(new object[] {"",idgenerado,txtDocumento.Text,txtRazonSocial.Text,txtCorreo.Text,txtTelefono.Text,
            ...
            {
                bool resultado = new Negocio_Proveedor().Editar(obj, out Mensaje);
                if (resultado)
                {
        ...
        private void dgwData_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgwData.Columns[e.ColumnIndex].Name == "btnSeleccionar") //aca le indicamos que si hizo click en el boton hara la siguiente serie de instrucciones
            {
                ...
                if (indice >= 0) // si se selecciono una fila valida
                {
                    ...
                    txtRazonSocial.Text = dgwData.Rows[indice].Cells["RazonSocial"].Value.ToString(); //
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Debe seleccionar algun Proveedor para eliminar.", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);   
            }
            else
            {
                if (MessageBox.Show("¿Desea eliminar el Proveedor?", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    bool respuesta = new Negocio_Proveedor().Eliminar(obj, out Mensaje);
